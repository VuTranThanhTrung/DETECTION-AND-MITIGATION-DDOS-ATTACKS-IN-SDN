...
class MitigationExecutor:
    ...
    def apply_hard_isolation(self, attacker_ip: str) -> bool:
        ...
        if success:
            self.restricted_ips.add(attacker_ip)
        return success

    def apply_rate_limiting(self, attacker_ip: str) -> bool:
        """
        Apply rate limiting by deleting existing flows of the attacker.
        This forces new packets to trigger packet-in and acquire meter limits.
    ...
    def remove_restrictions(self, attacker_ip: str) -> bool:
        ...
        for dp in list(self.datapaths.values()):
            try:
                ...
                # Delete outbound rules (including priority 100 DROP and priority 90/others)
                match_out = parser.OFPMatch(eth_type=0x0800, ipv4_src=attacker_ip)
                mod_out = parser.OFPFlowMod(
                    datapath=dp,
                    command=ofproto.OFPFC_DELETE,
